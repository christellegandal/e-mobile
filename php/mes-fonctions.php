...
function creerConnexionBDD()
{
    ...
    $database   = "e-mobile";   // NE PAS OUBLIER DE LE CHANGER
    $user       = "root";
    $password   = "";
    // Data Source Name
...
function lireTable($annonce
, $colonne="", $valeurSelection="", $tri="id DESC")
{
...
function concatenerTexteAsso ($annonce, $tabAssoColonneValeur)
{
    ...
    $texteFinal = "INSERT INTO $annonce ( $texteFinal ) VALUES ($texteToken)";
    return $texteFinal;
}
// INSERER UNE LIGNE DANS N'IMPORTE QUELLE TABLE
function insererLigneTable($annonce, $tabAssoColonneValeur)
{
    // ETAPE1: CREER UNE REQUETE SQL PREPAREE
    $requeteSQLPreparee = concatenerTexteAsso($annonce, $tabAssoColonneValeur);
...
function supprimerLigne($annonce, $id)
{
...
// exemple
// modifierLigne("annonce", "1", ["prix" => "nouveau prix", "marque" => "nouvelle marque", "nom" => "nouveau nom", "photo" => "nouvelle photo", "description" => "nouvelle description", "couleur" => "nouvelle couleur", "datePublication" => "nouvelle date de publication"]);
/*
...
datePublication = 'nouvelle date de publication'
WHERE 
id = 1;
...
id = 1;
*/
function modifierLigne($annonce, $id, $tabAssoColonneValeur)
{
    $id = intval($id);
    $listeColonneToken = "";
    // A VOUS LES STUDIOS... 
